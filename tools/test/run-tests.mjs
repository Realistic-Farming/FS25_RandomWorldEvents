// run-tests.mjs - offline logic tests for FS25_RandomWorldEvents.
//
...
for (const tf of testFiles) {
  ...
  for (const d of deps) {
    try {
      // Each dependency runs as its own chunk: RandomWorldEvents.lua ends with a
      // top-level return (legal in a source()d file), which would otherwise end the
      // concatenated program early. Globals still land in _G; file locals stay file-scoped.
