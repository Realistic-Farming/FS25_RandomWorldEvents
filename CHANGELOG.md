...
### Fixed
- Equipment Durability events (Low Wear / High Wear) now change the wear every vehicle in use takes while Arcade Physics is on: a vehicle driven by any player, run by a hired worker, or attached to one, decided on the server, on a dedicated server too. Parked vehicles are untouched. The old hook targeted a function that does not exist and never installed; the durability scaling now wraps each vehicle's own usage-damage step as it loads. Arcade events still show no notice to joined players, so on a dedicated server a wear event runs with no notice on screen.
